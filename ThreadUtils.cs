...
public static class ThreadUtils
{
    ...
    /// <summary>
    /// 带布尔返回值的异步执行函数，主要用于优化流程代码的可读性，返回值无意义
    /// </summary>
    ...
    /// <summary>
    /// 带布尔返回值和一个参数的异步执行函数，返回值主要用于优化流程代码的可读性，无意义
    /// </summary>
    /// <param name="func"></param>
    /// <returns></returns>
    public static Thread AsynExecuteWithResult<F>(Func<F, bool> func,F f)
    {
        Thread t = new Thread(() => {
            func.Invoke(f);
        });
    ...
    public static string SafeGetText(Control c) {
        string ret = "";
        if (c.InvokeRequired)
        {
            c.Invoke(new Action(() =>
