...
namespace EckyStudio.M.BaseModel.dmm.web
{
    public class EntityCommonModel
    {
        ...
        /// 获取DeepLink格式字符串签名
        /// </summary>
        /// <param name="type">签名算法，支持MD5和HMAC_SHA256和RSA</param>
        /// <param name="vars">身份关联key，为了增加签名的身份可靠性，一般会加入与身份关联的字符串，如密钥key，RSA私钥等</param>
            ...
            try
            {
                ...
                switch (type)
                {
                    case SignType.RSA:
                        ...
                        return RSAUtils.Sign(sb.ToString(), key_str);//直接返回RSA签名
                    case SignType.HMAC_SHA256:
                        sb.Append("key=" + key_str);
                        HMACSHA256 hmac = new HMACSHA256();
                        hash = hmac.ComputeHash(Encoding.UTF8.GetBytes(sb.ToString()));
        ...
        protected string GetDeepLink()
        {
            ...
            foreach (var m in members)
            {
                ...
                if (!string.IsNullOrEmpty(value))
                {
                    sb.Append(m.Key + '=' + System.Web.HttpUtility.UrlEncode(value) + '&');
                }
